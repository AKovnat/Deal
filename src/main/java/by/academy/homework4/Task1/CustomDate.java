...
public class CustomDate {
    ...
    public void calculateDays(CustomDate otherCustomDate) {
        ...
        LocalDate otherLocalDate = LocalDate.of(otherCustomDate.year.getYear(),
                otherCustomDate.month.getMonth(), otherCustomDate.day.getDay());
        System.out.println("Разница в днях между датами: " +
                Math.abs(localDate.toEpochDay() - otherLocalDate.toEpochDay()));
    }

    public void checkLeapYear() {
        final boolean isLeapYear = ((year.getYear() % 4 == 0) && (year.getYear() % 100 != 0) ||
                (year.getYear() % 400 == 0));

        if (isLeapYear) {
            System.out.println(year.getYear() + " - это високосный год");
        } else {
            System.out.println(year.getYear() + " - это невисокосный год");
        }
    }
}
